// services/fraud-service/internal/config/config.go
package config
...
func Load() *Config {
	return &Config{
		ServiceName:          getEnv("SERVICE_NAME", "fraud-service"),
		Environment:          getEnv("ENV", "development"),
		LogLevel:             getEnv("LOG_LEVEL", "info"),
		Version:              getEnv("SERVICE_VERSION", "1.0.0"),
		GRPCPort:             getEnv("GRPC_PORT", "5009"),
		...
		KafkaGroupID:         getEnv("KAFKA_GROUP_ID", "fraud-service"),
		KafkaTopicPrefix:     getEnv("KAFKA_TOPIC_PREFIX", "famgo"),
		HighRiskThreshold:    getEnvFloat("HIGH_RISK_THRESHOLD", 0.75),
		MediumRiskThreshold:  getEnvFloat("MEDIUM_RISK_THRESHOLD", 0.5),
		AnomalyCheckInterval: getEnvInt("ANOMALY_CHECK_INTERVAL_MIN", 60),
